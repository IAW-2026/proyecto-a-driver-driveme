      ...
      <main className="flex-1 min-w-0 pt-8 md:pt-28 pb-28 md:pb-8 px-4 md:px-10">
        <div className="w-full max-w-6xl mx-auto space-y-8">
          ...
          <HeaderModulo
            ...
            subtitulo={
              hayFiltros
                ? `${totalFiltrado} de ${totalGlobal} operador${totalGlobal !== 1 ? "es" : ""}`
                : `${totalGlobal} operador${totalGlobal !== 1 ? "es" : ""} en total`
            }
            acciones={
              <>
                <SignOutButton />
...
function SkeletonTabla() {
  return (
    <div className="rounded-modal border border-[rgba(255,255,255,0.06)] bg-[#141414] overflow-hidden animate-pulse">
      <div className="px-5 py-4 border-b border-[rgba(255,255,255,0.06)] h-24 bg-[#0A0A0A]" />
      <div className="p-5 space-y-3">
        {Array.from({ length: 5 }).map((_, i) => (
          <div
            key={i}
            className="h-12 rounded-sharp bg-[#1F1F1F]"
          />
        ))}
      </div>
    </div>
